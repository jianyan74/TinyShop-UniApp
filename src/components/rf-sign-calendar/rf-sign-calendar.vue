<template>
  <view class="rf-sign-calendar">
    <!-- 日历头部 -->
    <view v-if="signData.type === 'month'" class="r-header">
      <view v-if="state.indexMonth < (state.longList.length - 1)" class="r-header-icon prev" @tap="nextDate(1)">
        <text class="iconfont iconzuo font-weight-500" />
      </view>
      <view class="title">{{ state.showList.year }}年{{ state.showList.month }}月</view>
      <view v-if="state.indexMonth > 0" class="r-header-icon next" @tap="nextDate(-1)">
        <text class="iconfont iconyou" />
      </view>
    </view>
    <view v-if="signData.type === 'week'" class="r-header">
      <view v-if="state.weekIndex > 0" class="r-header-icon prev" @tap="nextWeekDate(-1)">
        <text class="iconfont iconzuo font-weight-500" />
      </view>
      <view :class="'text-' + themeColor.name" class="title">
        {{ state.yearTitle.year || '' }}年{{ state.yearTitle.month || '' }}月 第{{ state.yearTitle.week || '' }}周
      </view>
      <view v-if="state.weekIndex < (state.weekList.length - 1)" class="r-header-icon next" @tap="nextWeekDate(1)">
        <text class="iconfont iconyou font-weight-500" />
      </view>
    </view>
    <!-- 日历星期 -->
    ...
    <!-- 日历主体(月) -->
    <view v-if="signData.type === 'month'" class="r-content">
      <view class="r-content-box">
        <view v-for="(item, index) in state.showList.data" :key="index" class="item">
          <view class="flex-align-items">
            <rf-image v-if="item.isSign" :src="signInChecked" class="text" height="60" width="60" />
            <!--            <view v-if="item.isSign" class="text active">{{item.day}}</view>-->
            <view v-else-if="(Number(toMonth) === item.month) && (Number(today) === item.day)"
                  :class="'bg-' + themeColor.color"
                  class="text today">
              {{ item.day }}
            </view>
            <view v-else-if="state.showList.month === item.month" class="text tomonth">
              {{ item.day }}
            </view>
      ...
      <view class="info">
        <button :class="['bg-' + themeColor.name, todayIsSign ? 'sign-in-btn-disabled' : '']"
                class="sign-in-btn"
                @tap="handleSignCallback()">{{ todayIsSign ? '今日已签到' : '签到' }}
        </button>
      </view>
    </view>
    <!-- 日历主体(周) -->
    <view v-if="signData.type === 'week'" class="r-content">
      <swiper :current="state.weekIndex" :duration="300" class="week-box" @change="touchendWeek">
        <swiper-item v-for="(itemp, indexp) in state.weekList" :key="indexp">
          <view class="swiper-item">
            <view class="r-content-week">
              <view v-for="(item, index) in itemp" :key="index" class="item">
                <!--								#ifdef MP-->
                <view
                  v-if="state.ymd < `${item.year}${item.month < 10 ? `0${item.month}` : item.month}${item.day < 10 ? `0${item.day}` : item.day}`"
                  class="text after"
                ...
                <view :class="item.type === 'notday' ? 'after' : ''"
                      class="text"
                      @tap="signClick(index, 'week')"
                >{{ item.day }}
                </view>
                <view v-if="item.type === 'notday'"
                      class="text after"
                      @tap="signClick(index, 'week')"
                >{{ item.day }}
                </view>
                <!--								#endif-->
                <!--								#ifndef MP-->
                <view :class="dayClassFilter(item)"
                      class="text"
                      @tap="signClick(index, 'week')"
                >{{ item.day }}
                </view>
      ...
      <view class="info">
        <button :class="[todayIsSign ? 'sign-in-btn-disabled' : '']" class="sign-in-btn" @tap="handleSignCallback()">
          <text class="iconfont iconzhiwen" />
          {{ todayIsSign ? '今日已签到' : '签到' }}
        </button>
      </view>
    </view>
  </view>
...
watch(
  ...
  (newVal) => {
    ...
    if (props.signData.type == "week") {
      state.weekList[state.weekIndex].forEach((item) => {
        if (day === `${item.year}${item.month < 10 ? `0${item.month}` : item.month}${item.day < 10 ? `0${item.day}` : item.day}`) {
          item.isSign = true;
        }
      });
    ...
    if (props.signData.type == "month") {
      state.showList.data.forEach((item: any) => {
        if (day === `${item.year}${item.month < 10 ? `0${item.month}` : item.month}${item.day < 10 ? `0${item.day}` : item.day}`) {
          item.isSign = true;
        }
      });
...
const today = formatToDateTime(new Date(), "DD");
const toMonth = formatToDateTime(new Date(), "MM");
...
const nextDate = (index) => {
  ...
  if (state.indexMonth > state.longList.length - 1) {
    state.indexMonth = state.longList.length - 1;
  }
...
// 加载已签到信息
const signViewShow = () => {
  let days: any = state.showList.data, dayArr: any = props.signList;
  for (let i = 0, _i = days.length; i < _i; i++) {
    for (let j = 0, _j = dayArr.length; j < _j; j++) {
      if (dayArr[j].year == days[i].year && dayArr[j].month == (days[i].month.toString().length === 1 ? `0${days[i].month}` : days[i].month) && dayArr[j].day == days[i].day && days[i].type === "day") {
        days[i].isSign = true;
      }
    }
  }
  state.showList.data = days;
};
// 签到操作
  ...
  if (__z >= __n) {
    if (__z == __n) { // 当天签到
      if (!days.isSign) {
        days.isSign = true;
        // if(type == "month") {
        //   $set(showList.data, index, days);
        // }
        // if(type == "week") {
        //   $set(weekList[weekIndex], index, days);
        // }
        emit("callbackSign", {
          type: true,
          msg: "签到成功",
...
const touchendWeek = (e) => {
  ...
  if (state.nextIndex > state.weekIndex) {
    state.weekList = signViewShowWeek(state.weekList);
...
const getNowWeek = () => {
  ...
  state.nextIndex = state.weekIndex;
  state.weekList = signViewShowWeek(_);
  state.yearTitle = state.yearDay[state.weekIndex];
};
...
// 加载已签到信息
const signViewShowWeek = (data) => {
  let days = data[state.weekIndex], dayArr: any = props.signList;
  for (let i = 0, _i = days.length; i < _i; i++) {
    for (let j = 0, _j = dayArr.length; j < _j; j++) {
